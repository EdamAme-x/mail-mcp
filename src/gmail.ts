...
import type { Search } from "./mail.js";
...
export class Gmail {
  ...
  async get(id: string) {
    ...
    const visit = async (part: Part): Promise<void> => {
      ...
      if (
        part.filename ||
        /^attachment(?:\s*;|\s*$)/i.test(partHeader("content-disposition"))
      ) {
        attachments.push({
      ...
      if (part.mimeType === "text/plain" || part.mimeType === "text/html") {
        ...
        if (!data && part.body?.attachmentId) {
          const body = await this.request(
            `messages/${encodeURIComponent(id)}/attachments/${encodeURIComponent(part.body.attachmentId)}`,
            z.object({ data: z.string() })
          );
          ({ data } = body);
        }
        const charset = /(?:^|;)\s*charset\s*=\s*(?:"([^"]+)"|([^;\s]+))/i.exec(
          partHeader("content-type")
        );
        const decoded = new TextDecoder(
          charset?.[1] ?? charset?.[2] ?? "utf-8"
        ).decode(Buffer.from(data ?? "", "base64url"));
  ...
  async search(input: Search) {
    const query =
      input.query ?? (input.text ? JSON.stringify(input.text) : undefined);
