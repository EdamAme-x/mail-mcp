...
export async function login(
  ...
  const { installed: client } = clientSchema.parse(
  ...
  const listener = createServer((req, res) => {
    ...
    res.end(
      "Authorization received. Return to the terminal to check completion."
    );
    resolveCode(code);
  });
  ...
  if (!address || typeof address === "string") {
    throw new MailFault(failure("NETWORK"));
  }
  ...
  const timer = setTimeout(
    () => rejectCode(new MailFault(failure("TIMEOUT"))),
    300_000
  );
  try {
