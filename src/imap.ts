...
function decodeId(value: string) {
  if (!/^\d+:\d+$/.test(value)) throw new Error('Invalid IMAP message ID.')
  const [validity, uid] = value.split(':')
  const number = Number(uid)
  if (!Number.isSafeInteger(number) || number < 1 || number > 0xffffffff) throw new Error('Invalid IMAP message ID.')
...
export class Imap implements MailProvider {
  ...
  private async connected<T>(action: (client: ImapFlow, validity: string) => Promise<T>): Promise<T> {
    ...
    client.on('error', () => {}) // Network errors are handled by awaited operations, never logged with auth data.
    try {
      ...
      try {
        ...
        return await action(client, String(client.mailbox.uidValidity))
  ...
  async list({ text, limit = 20, pageToken }: Search) {
    return this.connected(async (client, validity) => {
      ...
      if (cursor && cursor.validity !== validity) throw new Error('IMAP mailbox changed. Start a new search.')
  ...
  async get(id: string) {
    ...
    return this.connected(async (client, validity) => {
      if (messageId.validity !== validity) throw new Error('IMAP mailbox changed. Search again for a new message ID.')
      const metadata = await client.fetchOne(String(messageId.uid), { size: true, uid: true }, { uid: true })
      if (!metadata) throw new Error('IMAP message no longer exists.')
      if ((metadata.size ?? 0) > maxMessageBytes) throw new Error('IMAP message exceeds the 10 MiB reading limit.')
      const m = await client.fetchOne(String(messageId.uid), { source: { maxLength: maxMessageBytes + 1 }, flags: true }, { uid: true })
      if (!m || !m.source) throw new Error('IMAP message no longer exists.')
      if (m.source.length > maxMessageBytes) throw new Error('IMAP message exceeds the 10 MiB reading limit.')
